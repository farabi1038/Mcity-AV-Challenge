...
class AVDecisionMakingModule(MRAVTemplateMcity):
    ...
    def derive_planning_result(self, step_info):
        ...
        for info in av_context_info.items():
            ...
            vehicle_info = info[1]
            if vehicle_info['leading_info']['is_leading_cav']:
                LV_xPos = vehicle_info['x']
                LV_yPos = vehicle_info['y']
                LV_longSpeed = vehicle_info['speed_long']
                LV_latSpeed = vehicle_info['speed_lat']
                ...
                AV_vel = AV_longSpeed + AV_latSpeed
            vehicle_info = info[1]
            if vehicle_info['lane_id']!=av_state['lane_id']:
                is_adj = True
                adj_xPos = vehicle_info['x']
                adj_yPos = vehicle_info['y']
                adj_longSpeed = vehicle_info['speed_long']
                adj_latSpeed = vehicle_info['speed_lat']
